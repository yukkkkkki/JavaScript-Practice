...
6. **定位布局**
   ...
   - position:absolute 绝对定位
     ...
     - 如果祖先元素有定位(除了 static 之外的任意值)，则**以最近一级的定位祖先元素为参考点**移动位置
...
22. **两边固定中间自适应**
    ...
    - 左右侧采用浮动 中间采用 margin-left 和 margin-right 方法

    - 左右两侧采用绝对定位 中间采用 margin-left margin-right 方法
...
21. **改变 this 的指向：call()、apply()、bind()**
...
3. **v-if 和 v-show**
   ...
   - **v-if** 用于条件性地渲染一块内容。这块内容只会在指令的表达式返回 tru 值的时候被渲染
