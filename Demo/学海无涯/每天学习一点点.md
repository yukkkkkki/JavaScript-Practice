...
12. **overflow**
    ...
    - overflow的作用：
      ...
      - 溢出隐藏
        ...
        给一个元素设置了overflow: hidden，那么该元素的内容若超出了给定的宽度和高度属性，那么超出的部分将会被隐藏，不占位
      ...
      - 清除浮动
        ...
        - 盒子塌陷：当父元素没有足够大小的时候，而子元素设置了浮动的属性，子元素就会跳出父元素的边界（脱离文档流），尤其是当父元素的高度为auto时，父元素没有其他非浮动的可见元素 ，父元素就会塌陷为零，我们称为CSS高度塌陷。
...
5. **http 状态码**
   ...
   |                        | 202: 已接受           | 303: 资源存在另一个URI（GET方法获取请求) | 403: 拒绝请求     | 503：或服务器正在停机维护 |
   |                        | 203: 成功，但未授权   | 304: 资源未修改，可使用缓存              | 404: 资源不存在   |                           |
   |                        | 204: 成功，无内容     | 305: 需代理访问305: 需代理访问           |                   |                           |
   |                        | 205: 成功，重置内容   | 307: 临时重定向，不会从POST变成GET       |                   |                           |
   |                        | 206: 成功，部分内容   |                                          |                   |                           |
...
1. **生命周期**
   ...
   | created       | 在实例创建完成后被立即调用，实例已完成数据观测，property 和方法的运算，watch/event 事件回调               |
...
4. **数据双向绑定**
   ...
   - **数据双向绑定原理**
     ...
     - 数据劫持Observer：对 `data`中的每一个属性值进行监测，只要数据变化了
       ...
       class Observer {
         ...
         observer(data) {
           ...
           if (!data || typeof data !== 'object') { //排除数据不存在或者不是对象的情况
             return;
           }
     ...
     - 观察者Watcher：新值和老值进行比对，如果发生变化，就调用更新方法，进行视图的更新
     ...
     - 总结
       ...
       - 通过defineProperty将属性转换成getter/setter的形式来追踪变化。读取数据时会触发getter，修改数据时会触发setter。我们需要在getter中收集有哪些依赖使用了数据，当getter被触发时，去通知getter中收集的依赖数据发生了变化。使用发布-订阅模式创建Dep，用来收集依赖、删除依赖和向依赖发送消息等。依赖其实就是指Watcher，只有Watcher触发的getter才会收集依赖，哪个Watcher触发了getter，就把哪个Watcher收集到Dep中。当数据发生变化时，会循环依赖列表，把所有的Watcher都通知一遍。Observer把一个object中的所有数据都转换成响应式的，即它会侦测到object中所有数据的变化。
...
- 添加一个具有唯一值的 key attribute，表达“这两个元素是完全独立的，不要复用它们”
  ...
  - v-if：是"真正"的条件渲染，因为它会确保在切换过程中条件块内的事件监听器和子组件适当地被销毁和重建。动态的向 DOM 树内添加或者删除 DOM 元素，若初始值为 `false` ，就不会编译了
...
9. **几种vue当中的指令和它的用法**
   ...
   | v-if      | 根据表达式的值的 truthiness来有条件地渲染元素。在切换时元素及它的数据绑定 / 组件被销毁并重建 |
...
11. **vue-loader**
    ...
    - vue-loader是基于webpack的一个loader，解析和转换.vue文件，提取出其中的逻辑代码script、样式代码style、以及HTML模板template，再分别把它们交给对应的loader去处理，核心的作用，就是提取。
...
12. **为什么使用key**
    ...
    - 如果不使用 key，Vue 会使用一种最大限度减少动态元素并且尽可能的尝试就地修改/复用相同类型元素的算法。而使用 key 时，它会基于 key 的变化重新排列元素顺序，并且会移除 key 不存在的元素
      ...
      - 一般设置key值得节点可以在diff中更快速地找到对应节点，提高diff速度
...
13. **axios及安装**
    ...
    - axios：是一个基于 promise 的 HTTP 库，用来请求后台资源的模块。

    - 安装：`npm install axios —save` 

    - 使用： js中`import` 进来，然后 `.get` 或 `.post` 。如果成功，返回在 `.then` 函数中，失败则是在 `.catch` 函数中。
    ...
    - API
      ...
      - 处理并发请求的助手函数
        ...
        - axios.all(iterable)：同时发起多个请求 -> 返回一个数组[res1, res2]

        - axios.spread(callback) -> 将数组[res1, res2]展开为res1, res2
...
16. **分别简述 computed 和 watch 的使用场景**
    ...
    - `computed `计算属性：类似于过滤器，对绑定到视图的数据进行处理，并监听变化进而执行对应的方法
      ...
      - 注意：
        ...
        - computed 依赖监控自己定义的变量，不能计算已经在data里面定义过的值，该变量在computed里面定义，然后可以在页面上进行数据绑定
    ...
    - `watch` 侦听属性：是一个侦听的动作，用来观察和响应 Vue 实例上的数据变动
      ...
      - 使用场景：当一条数据影响多条数据的时候
        - eg：搜索数据
      - 作用：
        - 监控vue实例的变化，监控的变量必须在data里面声明才可以
        - 监控一个变量或者一个对象，但是只能监控整个对象或单个变量
        - 依赖于固定的数据类型
      - watch 选项允许我们执行异步操作 ，限制我们执行该操作的频率，并在我们得到最终结果前，设置中间状态。这些都是计算属性无法做到的

      - 注意：通过vm对象的$watch()或watch配置来监视指定的属性，当属性变化时，回调函数自动调用，在函数内部进行计算。watch不能双向的绑定值
      ...
      - 区别
        ...
        - 不同：computed主要用于对同步数据的处理，watch则主要用于观测某个值的变化去完成一段开销较大的复杂业务逻辑。
...
18. **渐进式框架的理解**
    ...
    - Vue.js只提供了 vue-cli 生态中最核心的**组件系统** 和 **双向数据绑定（也叫数据驱动）**
