...
2.  **DNS 域名解析**
    ...
    - **工作流程**
      ...
      - 若 LDNS 仍未命中，则向**Root Server 域名服务器**发起请求解析这个域名
      - 根域名服务器返回给 LDNS 一个所查询域的**主域名服务器（**gTLD Server，国际顶尖域名服务器）
      - 此时 LDNS**向主域名服务器发起解析请求**
      - 主域名服务器接收到解析请求后，**查找并返回域名对应的域名服务器(Name Server)的地址**，这个 Name Server 就是**网站注册的域名服务器**
