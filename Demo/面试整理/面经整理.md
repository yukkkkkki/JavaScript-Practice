...
4. **CSS 选择器**
   ...
   - 伪类选择器(a:hover, li:nth-child)
     ...
     - 结构伪类（都以 span 为例）
       ...
       - :first-of-type 选择父元素下面第一个所匹配的子元素
       - :last-of-type 在有父元素的里面找最后一个匹配的子元素
       - :only-child 选择父元素下的"独生子"
         - 即看父元素下面的子元素是不是仅有一个，是的话就选中
       - :only-of-type 选择其父元素上的唯一一个相同类型的子元素

       - :nth-child(n) 选择父元素下面的第 n 个子元素

         - n 可以计算，css 里面的索引是从 1 开始的
         - :nth-child(odd) -> 奇数
         - :nth-child(even) -> 偶数

       - :nth-last-child(n) 选择父元素下倒数第 n 个子元素
       - :nth-of-type(n) 匹配父元素的第 n 个子元素
       - :nth-last-of-type(n) 匹配父元素的倒数第 n 个子元素
       - :not(selector) 选择所有 selector 以外的元素
